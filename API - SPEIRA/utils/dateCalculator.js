...
exports.obtenerRangoFechas = (periodo, fechaStr) => {
  ...
  if (periodo === 'semanal' && fecha.day() !== 1) {
    throw new Error('Para reportes semanales debe proporcionar una fecha que sea día lunes');
  }
  ...
  switch (periodo) {
    ...
    case 'semanal':
      return {
        fechaInicio: fecha.clone().startOf('week').toDate(),
        fechaFin: fecha.clone().endOf('week').toDate(),
        semanaStr: `${fecha.clone().format('YYYY-MM-DD')}_a_${fecha.clone().endOf('week').format('YYYY-MM-DD')}`
      };
    case 'mensual':
      return {
